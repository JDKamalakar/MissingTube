...
export const VideoGrid: React.FC<VideoGridProps> = ({ videos, filterMode = 'all' }) => {
  ...
  return (
    <>
      {/* Mobile-Optimized Sort Controls (unchanged from last iteration) */}
      ...
      >
        {sortedVideos.map((video) => (
          <div
            key={video.id}
            className="bg-white/30 dark:bg-black/40 backdrop-blur-heavy rounded-3xl overflow-hidden shadow-xl border border-white/30 dark:border-white/20 hover:shadow-2xl transition-all duration-300 group elevation-2 hover:elevation-4 flex flex-col" // This 'group' is for the whole card hover effect
          >
            ...
            <div className="p-3 sm:p-4 pt-2 flex-1 flex flex-col mobile-gap">
              ...
              <div className="flex gap-2 mt-auto">
                <button
                  ...
                  // Added 'group' to make THIS button a group for its own icon
                  className="flex-1 flex items-center justify-center gap-2 py-2 sm:py-3 px-2 sm:px-3 bg-secondary-container/20 backdrop-blur-lg rounded-2xl mobile-text-xs sm:text-xs font-medium hover:bg-secondary/90 transition-all duration-225 hover:scale-105 active:scale-95 shadow-sm hover:shadow-md touch-target group" 
                >
                  {/* Icon color and animation on group-hover of this specific button */}
                ...
                <button
                  onClick={() => handleVideoClick(video.videoId)}
                  // Added 'group' to make THIS button a group for its own icon
                  className="flex items-center justify-center p-2 sm:p-3 bg-primary-container/20 backdrop-blur-lg rounded-2xl hover:bg-primary/90 transition-all duration-225 hover:scale-110 active:scale-95 shadow-sm hover:shadow-md touch-target group" 
                >
                  {/* Icon color and animation on group-hover of this specific button */}
                ...
                <button
                  onClick={(e) => handleShowDescription(video, e)}
                  // Added 'group' to make THIS button a group for its own icon
                  className="hidden sm:flex p-2 sm:p-3 bg-tertiary-container/20 backdrop-blur-lg rounded-2xl shadow-md transition-all duration-300 hover:scale-[1.18] items-center justify-center active:scale-95 hover:shadow-lg touch-target group"
                >
                  {/* Icon color and animation on group-hover of this specific button */}
