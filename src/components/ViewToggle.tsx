  ...
  return (
    <>
      <div ref={dropdownRef} className={`relative w-full sm:w-auto ${isMobileMenuOpen ? 'z-20' : 'z-auto'}`}>
        ...
        {/* --- [MODIFIED] Mobile View (Dropdown with new style) --- */}
        <div className="sm:hidden w-full">
          ...
          {/* [MODIFIED] Dropdown container with new animation and styling */}
          <div className={`absolute top-full mt-2 left-0 right-0 bg-white/20 dark:bg-gray-800/20 backdrop-blur-xl border border-gray-300/30 dark:border-gray-700/30 rounded-xl shadow-xl p-2 flex flex-col gap-0.5 w-full transform transition-all duration-500 ease-out origin-top ${
            ...
            {viewOptions.map((option, index) => {
              ...
              const cornerClass = isFirst ? 'rounded-t-lg' : isLast ? 'rounded-b-lg' : '';

              return (
                <button
                  key={option.mode}
                  onClick={() => handleViewChange(option.mode)}
                  className={`group w-full flex items-center gap-3 px-4 py-3 transition-all duration-300 transform origin-center hover:scale-105 hover:-translate-y-1 text-sm font-medium ${cornerClass} ${
                ...
                >
                  <option.icon
                    ...
                    className={`transition-all duration-300 group-hover:scale-110 ${
                        option.mode === viewMode ? 'text-white scale-110' : 'text-primary'
                    }`}
                  />
