...
// [FIXED] Combined all lucide-react imports into one line
...
  currentPlaylistInfo = null
  ...
  // [FIXED] Moved hook to the correct position inside the component body
...
  ];
  ...
  // [FIXED] Moved themeOptions array to its own constant
  const themeOptions = [
...
        </div>
        ...
        {/* [FIXED] Corrected className and moved theme toggle UI inside */}
        <div className={`sm:hidden overflow-hidden transition-all duration-300 ease-in-out ${showMobileMenu ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
...
                  <button onClick={() => { item.onClick(); closeMobileMenu(); }} className="group relative flex items-center gap-4 w-full px-4 py-3 text-gray-900 dark:text-white rounded-2xl transition-all duration-300 hover:scale-[1.02] active:scale-98 h-11">
