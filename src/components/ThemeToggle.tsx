...
export const ThemeToggle: React.FC = () => {
  ...
  return (
    <div
      ref={themeToggleRef}
      // [MODIFIED] Adjusted positioning for a cleaner placement
      className={`fixed z-40 transition-all duration-300 ease-in-out ${
        isScrolled
          ? 'top-3 right-3' // Tucks into the corner when scrolled
          : 'top-4 right-4' // Sits a bit further out when not scrolled
      }`}
      style={{
    ...
    >
      <button
        ...
        // [MODIFIED] Simplified classes and added logic to change shape on scroll
        className={`flex items-center justify-center bg-white/25 dark:bg-gray-800/25 backdrop-blur-md border border-gray-300/40 dark:border-gray-700/40 hover:bg-white/30 dark:hover:bg-gray-800/30 transition-all duration-300 group shadow-xl touch-target active:scale-95 hover:scale-110 ${
          isScrolled
            ? 'w-12 h-12 rounded-full' // When scrolled, become a larger circle (width equals height)
            : 'p-2 sm:p-3 rounded-xl sm:rounded-2xl' // Default shape
        }`}
        aria-label="Toggle theme"
      >
        {/* [MODIFIED] Icon container now resizes based on scroll state */}
