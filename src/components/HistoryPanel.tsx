      ...
      >
        ...
        <div className="p-4 sm:p-8">
          {playlists.length === 0 ? (
            ...
            // [MODIFIED] Restored grid layout for individual cards
            <div className="grid gap-3 sm:gap-4">
              {playlists.map((playlist) => (
                // [MODIFIED] Card is now a self-contained button with its own background, border, and shadow
                <button
                  key={playlist.id}
                  onClick={() => handlePlaylistClick(playlist.id)}
                  className="group relative w-full text-left bg-white/20 dark:bg-gray-800/20 backdrop-blur-lg rounded-2xl p-3 sm:p-4 transition-all duration-300 ease-out border border-gray-300/30 dark:border-gray-700/30 shadow-sm hover:shadow-xl hover:scale-105 hover:-translate-y-1 active:scale-[0.98] z-0 hover:z-10"
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div className="flex flex-shrink-0 items-center gap-1 sm:gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-225">
                      <button
                        ...
                        className="p-1.5 sm:p-2 text-on-surface-variant hover:text-primary hover:bg-primary-container rounded-full transition-all duration-300 hover:scale-125 active:scale-95"
