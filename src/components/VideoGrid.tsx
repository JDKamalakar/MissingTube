...
export const VideoGrid: React.FC<VideoGridProps> = ({ videos, filterMode = 'all' }) => {
  ...
  const getSortIcon = (field: SortField) => {
    if (sortField !== field) return <ArrowUpDown className="w-4 h-4 opacity-50 text-gray-900 dark:text-white" />;
    return sortDirection === 'asc' ? 
      <ArrowUp className="w-4 h-4 text-primary dark:text-white" /> : 
      <ArrowDown className="w-4 h-4 text-primary dark:text-white" />;
  };
  ...
  return (
    <>
      {/* Sort Controls - MODIFIED */}
      <div className="flex items-center gap-4 mb-6 p-4 bg-white/30 dark:bg-black/40 backdrop-blur-heavy rounded-3xl border border-white/30 dark:border-white/20 elevation-2 group hover:scale-[1.01] transition-transform duration-300">
      ...
      >
        {sortedVideos.map((video) => (
          <div
            key={video.id}
            className="bg-white/30 dark:bg-black/40 backdrop-blur-heavy rounded-3xl overflow-hidden shadow-xl border border-white/30 dark:border-white/20 hover:shadow-2xl transition-all duration-300 group elevation-2 hover:elevation-4 flex flex-col hover:scale-[1.02]"
          >
            ...
            <div className="relative p-4 pb-2">
              <div className="relative bg-white/20 dark:bg-black/20 rounded-2xl overflow-hidden">
                ...
                {/* Duration badge */}
                <div className="flex items-center absolute bottom-2 right-2 gap-1 bg-black/60 text-white backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 elevation-2 px-3 py-3 text-sm hover:scale-110 transition-transform duration-225">
                  <Clock className="w-3 h-3 text-white" />
                  {video.duration}
                </div>

                {/* Index number */}
                <div className="absolute top-2 left-2 flex items-center gap-1 bg-black/60 text-white backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 elevation-2 px-3 py-3 text-xs hover:scale-110 transition-transform duration-225">
                  #{video.index}
                </div>
              </div>
            </div>
